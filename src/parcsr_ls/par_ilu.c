 ...
 */
HYPRE_Int
hypre_ILUGetInteriorExteriorPermDevice(hypre_ParCSRMatrix *A, HYPRE_Int **perm, HYPRE_Int *nLU,
                                       HYPRE_Int reordering_type)
{
   ...
   if (reordering_type)
   {
      ...
      hypre_ILULocalRCMBuildG(A_diag, 0, first, &temp_perm, &temp_perm, &G);
      ...
      {
         case 1: //RCM
            HYPRE_CUSOLVER_CALL(cusolverSpXcsrsymrcmHost(handle, G_nrows, G_nnz, descr,
                                G_i, G_j, G_perm));
            break;
         case 2: // AMD
            HYPRE_CUSOLVER_CALL(cusolverSpXcsrsymamdHost(handle, G_nrows, G_nnz, descr,
                                G_i, G_j, G_perm));
            break;
         case 3: // MDQ
            HYPRE_CUSOLVER_CALL(cusolverSpXcsrsymmdqHost(handle, G_nrows, G_nnz, descr,
                                                         G_i, G_j, G_perm));
            break;
         case 4: // MetisND
            HYPRE_CUSOLVER_CALL(cusolverSpXcsrmetisndHost(handle, G_nrows, G_nnz, descr,
                                                          G_i, G_j, NULL, G_perm));
            break;
      }
       ...
       */
      hypre_ILULocalRCMBuildFinalPerm(0, first, G_perm, temp_perm, temp_perm, &temp_perm, &temp_perm);
   ...
   *perm = temp_perm;
   ...
   cudaEventSynchronize( estop );
   cudaEventElapsedTime( &time, estart, estop );
   printf("%s %s %d : Reordering time=%1.5g\n",__FILE__,__FUNCTION__,__LINE__,time/1000.);
...
HYPRE_Int
hypre_ILUGetLocalPermDevice(hypre_ParCSRMatrix *A, HYPRE_Int **perm, HYPRE_Int *nLU,
                            HYPRE_Int reordering_type)
{
   /* get basic information of A */
   hypre_CSRMatrix      *A_diag = hypre_ParCSRMatrixDiag(A);
   ...
   switch (reordering_type)
   {
      case 0: default: //None
         ...
         break;
      case 1: //RCM
         HYPRE_CUSOLVER_CALL(cusolverSpXcsrsymrcmHost(handle, n, nnz, descr,
                                                      A_i, A_j, tperm));
         break;
      case 2: // AMD
         HYPRE_CUSOLVER_CALL(cusolverSpXcsrsymamdHost(handle, n, nnz, descr,
                                                      A_i, A_j, tperm));
         break;
      case 3: // MDQ
         HYPRE_CUSOLVER_CALL(cusolverSpXcsrsymmdqHost(handle, n, nnz, descr,
                                                      A_i, A_j, tperm));
         break;
      case 4: // MetisND
         HYPRE_CUSOLVER_CALL(cusolverSpXcsrmetisndHost(handle, n, nnz, descr,
                                                       A_i, A_j, NULL, tperm));
         break;
   }
                      ...
                      HYPRE_Int reordering_type)
{
   ...
   if (n > 0)
   {
      HYPRE_THRUST_CALL( sequence, temp_perm, temp_perm+n, 0 );
   }
#else
   for ( i = 0 ; i < n ; i ++ )
   {
...
/*--------------------------------------------------------------------------
 * hypre_ParILUCusparseSchurGMRESMatvecJacobiIter
 *--------------------------------------------------------------------------*/

HYPRE_Int
hypre_ParILUCusparseSchurGMRESMatvecJacobiIter( void   *matvec_data,
                                                HYPRE_Complex  alpha,
                                                void   *ilu_vdata,
                                                void   *x,
                                                HYPRE_Complex  beta,
