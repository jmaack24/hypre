...
const HYPRE_Real STATIC_PIVOT_TOL = 1e-8;
const HYPRE_Real IS_ZERO_TOL = 1e-13;
...
HYPRE_Int hypre_lcsc_sparse_col_to_dense(HYPRE_Int k,
...
HYPRE_Int hypre_ldl_update_lower(HYPRE_Int k,
				 HYPRE_Int *Lcsc_col_offsets,
				 HYPRE_Int *Lcsc_rows,
				 ...
				 HYPRE_Real *x)
{
  ...
  // Compute
  // x -= Sum_{i=0}^{k-1} L(k,i) * D(i,i) * L(k:n, i)
  ...
  for (i = 0; i < k; ++i)
    {
      factor = 0.0;
      for (j = Lcsc_col_offsets[i]; j < Lcsc_col_offsets[i+1]; ++j)
	{
      ...
      for (j = Lcsc_col_offsets[i]; j < Lcsc_col_offsets[i+1]; ++j)
	{
	  if (Lcsc_rows[j] >= k)
	    {
	      x[Lcsc_rows[j]] -= factor * Lcsc_data[j];
	    }
	}
    }
...
{
  const HYPRE_Real *v;
  by_value(const HYPRE_Real *vec) : v(vec) {}
  inline bool operator()(const HYPRE_Int &a, const HYPRE_Int &b) const
  {
    return fabs(v[a]) > fabs(v[b]);
  }
};
...
{
  const HYPRE_Real *v;
  const HYPRE_Real eps;
  by_tolerance(const HYPRE_Real *vec, HYPRE_Real eps) : v(vec), eps(eps) {}
  inline bool operator()(const HYPRE_Int &a) const
  {
    return fabs(v[a]) < eps;
  }
};

HYPRE_Int hypre_ldl_drop(HYPRE_Int k,
			 HYPRE_Int n,
			 HYPRE_Real tol,
			 HYPRE_Int lfil,
			 thrust::host_vector<HYPRE_Int> curr_nnzs,
			 HYPRE_Real *work)
{
  ...
  HYPRE_Real tolerance = tol * mag;
  if (tolerance > IS_ZERO_TOL)
    {
      for (auto it = curr_nnzs.begin(); it != curr_nnzs.end(); ++it)
	{
	  if (fabs(work[*it]) < tolerance)
	    {
	      work[*it] = 0;
	    }
	}

      by_value sorter(work);
      thrust::sort(curr_nnzs.begin(), curr_nnzs.end(), sorter);
    }

  for (HYPRE_Int i = lfil; i < (int) curr_nnzs.size(); ++i)
    {
      work[curr_nnzs[i]] = 0;
    }

  by_tolerance is_zero(work, IS_ZERO_TOL);
  curr_nnzs.erase(remove_if(curr_nnzs.begin(), curr_nnzs.end(), is_zero),
		  curr_nnzs.end());

  hypre_assert((int) curr_nnzs.size() <= lfil);

  thrust::sort(curr_nnzs.begin(), curr_nnzs.end());
...
/*    // HYPRE_Int lfil = fill_factor*(nnz_A - m) / m; */
/*    HYPRE_Int lfil = fill_factor; */
...
HYPRE_Int
hypre_ILUSetupILDLTNoPivot(hypre_CSRMatrix *A_diag,
			   HYPRE_Int lfil,
			   HYPRE_Real *tol,
                           HYPRE_Int *permp,
			   HYPRE_Int *qpermp,
			   ...
			   HYPRE_Int *ldl_nnz)
{
  ...
  HYPRE_Int i=0, j=0, k=0;
  HYPRE_Int Lcsc_nnz=0, col_k_nnz;
  HYPRE_Real dk;
  ...
  */
  ...
  HYPRE_Real * work = hypre_CTAlloc(HYPRE_Real, n, HYPRE_MEMORY_HOST);
  HYPRE_Real * column_vals = hypre_CTAlloc(HYPRE_Real, n, HYPRE_MEMORY_HOST);
  thrust::host_vector<HYPRE_Int> curr_nnzs;
  curr_nnzs.reserve(n);
  thrust::host_vector<HYPRE_Int> final_nnzs;
  final_nnzs.reserve(n);

  /*************************************************************************/
  /* Loop the remaining columns                                       */
  /*************************************************************************/

  for (k=0; k<n; ++k) {

    // Clear work space
    hypre_Memset(work, 0, sizeof(HYPRE_Real)*n, HYPRE_MEMORY_HOST);
    curr_nnzs.clear();
    final_nnzs.clear();

    // Convert column k of A matrix to dense vector
    hypre_lcsc_sparse_col_to_dense(k, Acsc_col_offsets, Acsc_rows, Acsc_data, work);

    /************/
    /* Update L */
    /************/

    hypre_ldl_update_lower(k, Lcsc_col_offsets, Lcsc_rows, Lcsc_data, D_data, work);
    dk = work[k];
    ...
    // Collect all nonzeros row values in the current work vector excluding the diagonal
    // to avoid dropping it
    for (i = k+1; i < n; ++i)
      {
	if(work[i] != 0.0)
	  {
	    curr_nnzs.push_back(i);
	  }
      }

    hypre_ldl_drop(k, n, tol[0], lfil, curr_nnzs, work);
    ...
    // Static pivot
    if (fabs(dk) < STATIC_PIVOT_TOL)
      dk = STATIC_PIVOT_TOL;

    D_data[k] = dk;
    column_vals[0] = 1;
    final_nnzs.push_back(k);
    col_k_nnz = 1;
    for (auto it = curr_nnzs.begin(); it != curr_nnzs.end(); ++it)
      {
	if(fabs(work[*it]) > STATIC_PIVOT_TOL)
	  {
	    column_vals[col_k_nnz] = work[*it] / dk;
	    final_nnzs.push_back(*it);
	    ++col_k_nnz;
	  }
      }

    hypre_assert(col_k_nnz == (int) final_nnzs.size());
    ...
    Lcsc_col_offsets[k+1] = Lcsc_col_offsets[k] + col_k_nnz;
    for (i = Lcsc_col_offsets[k], j = 0; i < Lcsc_col_offsets[k+1]; ++i, ++j)
      {
	Lcsc_rows[i] = final_nnzs[j];
	Lcsc_data[i] = column_vals[j];
      }
  ...
  hypre_TFree(column_vals, HYPRE_MEMORY_HOST);
