                         ...
                         hypre_CSRMatrix **Fptr, HYPRE_Int **A_fake_diag_ip, HYPRE_Int tri_solve)
{
   ...
   for (i = 0; i < n; i++)
   {
      rperm_host[perm_host[i]] = i;
      rqperm_host[qperm_host[i]] = i;
   }
#endif
   ...
   if (n > 0)
   {
      ...
      /* Prune the smallest values out of matrix */
      //HYPRE_ILUSetupCusparsePruneByPercentage(A_diag);
...
HYPRE_ILUSetupRocsparseCSRILU0(hypre_CSRMatrix *A, rocsparse_analysis_policy analysis_policy, rocsparse_solve_policy solve_policy)
{
   ...
   if (isDoublePrecision)
   {
      HYPRE_ROCSPARSE_CALL(rocsparse_dcsrilu0_buffer_size(handle, n, nnz_A, descr, (hypre_double *) A_data, A_i, A_j,
                                                          info, &buffer_size));
   }
   else if (isSinglePrecision)
   {
      HYPRE_ROCSPARSE_CALL(rocsparse_scsrilu0_buffer_size(handle, n, nnz_A, descr, (float *) A_data, A_i, A_j,
                                                          info, &buffer_size));
   }
   ...
   if (isDoublePrecision)
   {
      HYPRE_ROCSPARSE_CALL(rocsparse_dcsrilu0_analysis(handle,  n, nnz_A, descr, (hypre_double *) A_data, A_i, A_j,
                                                       info, analysis_policy, solve_policy, buffer));
   }
   else if (isSinglePrecision)
   {
      HYPRE_ROCSPARSE_CALL(rocsparse_scsrilu0_analysis(handle,  n, nnz_A, descr, (float *) A_data, A_i, A_j,
                                                       info, analysis_policy, solve_policy, buffer));
   }
   /* 5-2. Check for zero pivot */
   ...
   if (isDoublePrecision)
   {
      HYPRE_ROCSPARSE_CALL(rocsparse_dcsrilu0(handle, n, nnz_A, descr, (hypre_double *) A_data, A_i, A_j,
                                              info, solve_policy, buffer));
   }
   else if (isSinglePrecision)
   {
      HYPRE_ROCSPARSE_CALL(rocsparse_scsrilu0(handle, n, nnz_A, descr, (float *) A_data, A_i, A_j,
                                              info, solve_policy, buffer));
   }
                                        ...
                                        hypre_CsrsvData ** matLU_csrsvdata_ptr)
{
   ...
   if (isDoublePrecision)
   {
      ...
      HYPRE_CUSPARSE_CALL(cusparseDcsrsv2_bufferSize(handle, CUSPARSE_OPERATION_NON_TRANSPOSE, n, nnz_A,
                                                     hypre_GpuMatDataMatDescr(matL_des), (hypre_double *) A_data, A_i, A_j,
                                                     matL_info, &matL_buffersize));

      HYPRE_CUSPARSE_CALL(cusparseDcsrsv2_bufferSize(handle, CUSPARSE_OPERATION_NON_TRANSPOSE, n, nnz_A,
                                                     hypre_GpuMatDataMatDescr(matU_des), (hypre_double *) A_data, A_i, A_j,
   ...
   else if (isSinglePrecision)
   {
      ...
      HYPRE_CUSPARSE_CALL(cusparseScsrsv2_bufferSize(handle, CUSPARSE_OPERATION_NON_TRANSPOSE, n, nnz_A,
                                                     hypre_GpuMatDataMatDescr(matL_des), (float *) A_data, A_i, A_j,
                                                     matL_info, &matL_buffersize));

      HYPRE_CUSPARSE_CALL(cusparseScsrsv2_bufferSize(handle, CUSPARSE_OPERATION_NON_TRANSPOSE, n, nnz_A,
                                                     hypre_GpuMatDataMatDescr(matU_des), (float *) A_data, A_i, A_j,
                                                     matU_info, &matU_buffersize));
   }
                                         ...
                                         hypre_CsrsvData ** matLU_csrsvdata_ptr)
{
   ...
   if (isDoublePrecision)
   {
      ...
      HYPRE_ROCSPARSE_CALL(rocsparse_dcsrsv_buffer_size(handle, rocsparse_operation_none, n, nnz_A,
                                                        hypre_GpuMatDataMatDescr(matL_des), (hypre_double *) A_data, A_i, A_j,
                                                        matL_info, &matL_buffersize));
   ...
   else if (isSinglePrecision)
   {
      ...
      HYPRE_ROCSPARSE_CALL(rocsparse_scsrsv_buffer_size(handle, rocsparse_operation_none, n, nnz_A,
                                                        hypre_GpuMatDataMatDescr(matL_des), (float *) A_data, A_i, A_j,
                                                        matL_info, &matL_buffersize));
   ...
   if (isDoublePrecision)
   {
      ...
      HYPRE_ROCSPARSE_CALL( rocsparse_dcsrsv_analysis(handle, rocsparse_operation_none,
                                                      n, nnz_A, hypre_GpuMatDataMatDescr(matL_des),
                                                      (hypre_double *) A_data, A_i, A_j, matL_info,
                                                      ilu_analysis_policy, ilu_solve_policy, solve_buffer));

      HYPRE_ROCSPARSE_CALL( rocsparse_dcsrsv_analysis(handle, rocsparse_operation_none,
                                                      n, nnz_A, hypre_GpuMatDataMatDescr(matU_des),
                                                      (hypre_double *) A_data, A_i, A_j, matU_info,
                                                      ilu_analysis_policy, ilu_solve_policy, solve_buffer));
   }
   else if (isSinglePrecision)
   {
      ...
      HYPRE_ROCSPARSE_CALL( rocsparse_scsrsv_analysis(handle, rocsparse_operation_none,
                                                      n, nnz_A, hypre_GpuMatDataMatDescr(matL_des),
                                                      (float *) A_data, A_i, A_j, matL_info,
                                                      ilu_analysis_policy, ilu_solve_policy, solve_buffer));

      HYPRE_ROCSPARSE_CALL( rocsparse_scsrsv_analysis(handle, rocsparse_operation_none,
                                                      n, nnz_A, hypre_GpuMatDataMatDescr(matU_des),
                                                      (float *) A_data, A_i, A_j, matU_info,
                                                      ilu_analysis_policy, ilu_solve_policy, solve_buffer));
   }
...
{
#if defined(HYPRE_USING_CUSPARSE)
   hypre_ParILUCusparseILUExtractEBFC(A_diag, nLU, Bp, Cp, Ep, Fp);
#endif

#if defined(HYPRE_USING_ROCSPARSE)
   hypre_ParILURocsparseILUExtractEBFC(A_diag, nLU, Bp, Cp, Ep, Fp);
#endif
   return hypre_error_flag;
}
...
HYPRE_Int
hypre_ParILURocsparseILUExtractEBFC(hypre_CSRMatrix *A_diag, HYPRE_Int nLU, hypre_CSRMatrix **Bp,
                                   hypre_CSRMatrix **Cp, hypre_CSRMatrix **Ep, hypre_CSRMatrix **Fp)
{
   /* Get necessary slots */
   HYPRE_Int           *A_diag_i       = hypre_CSRMatrixI(A_diag);
