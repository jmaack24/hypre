...
HYPRE_Int hypre_IJVectorAssembleSortAndReduce0(HYPRE_Int  N, HYPRE_Int N0, HYPRE_BigInt *I0, HYPRE_Complex *A0, HYPRE_Int *N1, HYPRE_BigInt **I1, HYPRE_Complex **A1 );
...
__global__ void hypreCUDAKernel_IJVectorAssembleParSet(HYPRE_Int m,HYPRE_Int n, HYPRE_Complex *x, HYPRE_BigInt *map, HYPRE_BigInt offset, HYPRE_Complex *y);
                                    ...
                                    const char           *action)
{
       ...
       hypre_AuxParVectorUsrOffProcRecvElmts(aux_vector)>=0)
   {
      ...
      //printf("%s %s %d : on=%d, send=%d, recv=%d, current=%d, filled=%d\n",__FILE__,__FUNCTION__,__LINE__,
      //     stack_elmts_on_proc,stack_elmts_off_proc_send,stack_elmts_off_proc_recv,stack_elmts_current,usr_elmts_filled);
   ...
   if (nelms_on>=0 && nelms_off_recv>=0 && nelms_off_send>=0)
   {
	 ...
	 /* enforce this here */
      ...
      if (nelms)
      {
         ...
         /* sort and reduce */
         hypre_IJVectorAssembleSortAndReduce0(nelms_on, nelms, stack_i, stack_data, &new_nnz, &new_i, &new_data);
	 ...
	 hypre_TMemcpy(hypre_VectorData(hypre_ParVectorLocalVector(par_vector)), new_data, HYPRE_Complex, nelms_on, HYPRE_MEMORY_DEVICE, HYPRE_MEMORY_DEVICE);
         ...
         HYPRE_CUDA_LAUNCH( hypreCUDAKernel_IJVectorAssembleParSet, gDim, bDim, nelms_on, new_nnz-nelms_on, new_data, new_i, vec_start,
 ...
 */
HYPRE_Int
hypre_IJVectorAssembleSortAndReduce0(HYPRE_Int  N, HYPRE_Int  N0, HYPRE_BigInt  *I0, HYPRE_Complex  *A0,
 ...
 * same index cannot appear more than once in map */
__global__ void
hypreCUDAKernel_IJVectorAssembleParSet(HYPRE_Int m, HYPRE_Int n, HYPRE_Complex *x, HYPRE_BigInt *map, HYPRE_BigInt offset, HYPRE_Complex *y)
{
   HYPRE_Int i = hypre_cuda_get_grid_thread_id<1,1>();
